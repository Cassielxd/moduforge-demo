...
use crate::{commands::{fbfx_csxm::{DeleteFbfxCsxmCommand, InsertFbfxCsxmCommand}, AddRequest, DeleteNodeRequest}, controller::GcxmTreeItem, error::AppError, nodes::fbfx_csxm::{DE_RCJ_STR, DE_STR, FB_STR, QD_STR}, res, response::Res, ContextHelper, ResponseResult};
...
pub async fn add_fbfx_csxm(Json(mut param): Json<AddRequest>) -> ResponseResult<String> {
    ...
    editor.command_with_meta(Arc::new(InsertFbfxCsxmCommand{data:param.clone()}), "插入 分部分项 节点".to_string(), meta)
        .await?;
...
pub async fn delete_fbfx_csxm(Json(param): Json<DeleteNodeRequest>) -> ResponseResult<String> {
    ...
    editor.command_with_meta(Arc::new(DeleteFbfxCsxmCommand{data:param.clone()}), "删除 分部分项 节点".to_string(), meta)
        .await?;
    res!("success".to_string())
}
...
pub async fn get_fbfx_csxm_tree(Json(param): Json<FbfxCsxmPost>) -> ResponseResult<GcxmTreeItem> {
    ...
    if node.is_none() {
        return Err(AppError(anyhow::anyhow!("分部分项 措施项目 跟节点不存在".to_string())));
    }
    let node = node.unwrap();
    let mut nodes: Vec<Arc<Node>> = doc.descendants(&param.id).iter().filter(|n| n.r#type == FB_STR || n.r#type == QD_STR || n.r#type == DE_STR || n.r#type == DE_RCJ_STR).cloned().collect();
    nodes.push(node);
    let parent_map = &doc.get_inner().parent_map;
    if let Some(root_item) = GcxmTreeItem::from_nodes(param.id, nodes, parent_map) {
        res!(root_item)
    } else {
        Err(AppError(anyhow::anyhow!("无法构建工程树,未找到分部分项 措施项目 跟节点")))
    }
}
